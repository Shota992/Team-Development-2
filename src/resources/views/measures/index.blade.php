                ...
                <table class="border-collapse table-auto w-full mt-8">
                    <thead>
                        <tr class="bg-gray-100 text-gray-700">
                            <th class="sticky z-30 left-0 bg-white px-4 py-2 w-32 border-r-white border border-gray-300" width="200"><p class="w-60">件名</p></th>
                            <th class="sticky z-30 left-[272px] bg-white px-4 py-2 w-32 border border-gray-300" width="200"><p class="w-60 pl-20">担当者</p></th>
                            @foreach ($dateList as $date)
                                <th class="px-2 py-2 border border-gray-300 bg-white text-center">{{ \Carbon\Carbon::parse($date)->format('m/d') }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($measures as $measure)
                            ...
                            <tr class="measure-row border-b border-gray-300">
                                <td class="sticky z-30 left-0 bg-white px-4 py-2 font-bold cursor-pointer border border-gray-300"
                                    colspan="2">
                                ...
                                @foreach ($dateList as $date)
                                    <td id="measure{{ $measure->id }}_{{ $date }}" class="border border-gray-300 bg-white"></td>
                                @endforeach
                            </tr>
                            {{-- タスク（task）の行 - デフォルトで非表示 --}}
                            @foreach ($measure->tasks as $task)
                                <tr class="task-row tasks-{{ $measure->id }} hidden border-b">
                                    <td class="sticky z-30 left-0 bg-white px-4 py-2 border border-gray-300" colspan="2">
                                    <div class="grid grid-cols-4 ">
                                        <p class="col-span-3">{{ $task->name }}</p>
                                    ...
                                    @foreach ($dateList as $date)
                                        ...
                                        @endphp
                                        <td id="task{{ $task->id }}_{{ $date }}" class="border bg-white border-gray-300 text-center">
                                            @if ($isWithinRange)
